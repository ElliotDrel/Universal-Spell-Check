...
internal sealed class SettingsForm : Form
{
    ...
    private readonly CheckBox _useEnvCheckBox = new();
    ...
    public SettingsForm(SettingsStore settingsStore, DiagnosticsLogger logger)
    {
        ...
        _useEnvCheckBox.Text = "Use OPENAI_API_KEY environment variable";
        _useEnvCheckBox.AutoSize = true;
        _useEnvCheckBox.Location = new Point(16, 80);
        _useEnvCheckBox.Checked = settings.UseEnvApiKey;
        _useEnvCheckBox.CheckedChanged += OnUseEnvCheckBoxChanged;
        UpdateApiKeyEnabled(!settings.UseEnvApiKey);
        ...
        {
            Text = "Close",
            Location = new Point(429, 118),
            Size = new Size(75, 30)
        };
        closeButton.Click += (_, _) => Close();

        _statusLabel.AutoSize = false;
        _statusLabel.Location = new Point(16, 156);
        _statusLabel.Size = new Size(488, 24);

        Controls.Add(apiKeyLabel);
        Controls.Add(_apiKeyTextBox);
        Controls.Add(_useEnvCheckBox);
        Controls.Add(saveButton);
        Controls.Add(closeButton);
        Controls.Add(_statusLabel);
    }

    private void OnUseEnvCheckBoxChanged(object? sender, EventArgs e)
    {
        var useEnv = _useEnvCheckBox.Checked;
        var settings = _settingsStore.Load();
        settings.UseEnvApiKey = useEnv;
        _settingsStore.SaveSettings(settings);
        UpdateApiKeyEnabled(!useEnv);
        _statusLabel.Text = useEnv ? "Reading from OPENAI_API_KEY environment variable." : "";
        _logger.Log($"use_env_api_key_toggled value={useEnv}");
    }

    private void UpdateApiKeyEnabled(bool enabled)
    {
