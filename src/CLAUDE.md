...
14. Start a correction, then switch to another window owned by the same executable before the request completes. Verify no paste occurs and the original clipboard is restored. An owned IME/autocomplete window with the same root owner must not cause a false rejection.
