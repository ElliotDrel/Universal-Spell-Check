...
- Bump `scriptVersion` before commits that include `Universal Spell Checker.ahk`
...
- Pre-commit must reject commits that include `Universal Spell Checker.ahk` when the staged `scriptVersion` is not a numeric string or does not increase relative to `HEAD`
